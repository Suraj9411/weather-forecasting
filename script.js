...
function getWeatherData(city, unit, hourlyorWeek) {
    const apiKey = "9Y3V5SYF87HSV373JREAZAYPC";
    ...
    .then((data) => {
        ...
        if(unit === "c") {
            ...
            temp.innerText = celciusToFehrenheit(today.temp);
        }
...
function celciusToFehrenheit(temp) {
    return ((temp * 9) / 5 + 32).toFixed(1);
}

//function to get uv uindex status
...
function updateHumidityStatus(humudity) {
    if(humudity <= 30) {
...
function updateVisibilityStatus(visibility) {
    if(visibility <= 0.3) {
        ...
        visibilityStatus.innerText = "Very High Fog";
...
function getIcon(condition) {
    if(condition === "Parity-cloudy-day") {
        return "/icons/5.png";
    } else if(condition === "Parity-cloudy-night") {
        return "/icons/6.png";
...
function updateForecast(data, unit, type) {
    ...
    for(let i=0; i<numCards; i++) {
        ...
        if(unit === "f") {
            dayTemp = celciusToFehrenheit(data[day].temp);
        }
...
search.addEventListener("input", function (e) {
    ...
    // if there is nothing then search input will do nothing
    if(!val) {
        return false;
    } 
    currentFocus = -1;

    //creating a ul with a id sugggstion
    ...
    for(i=0; i<cities.length; i++) {
        ...
        if(cities[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
            ...
            // remaining part of the suggestion
            b.innerHTML += cities[i].substr(val.length);
            //inpput field to hold the suggestion value
            b.innerHTML += "<input type='hidden' value='" + cities[i] + "'>";

            //adding eventlistener on suggestion
            b.addEventListener("click", function (e) {
                //on click set the search inout vlaue with the clicked suggestion vlaue
...
//to remove the suugestion coming in above another
function removeSuggestions() {
    //select trhe ul which is being adding on search input
    var x = document.getElementById("suggestions");
    //if x exists remove it
    if(x) x.parentNode.removeChild(x);
}

//add up and down key functionality 

search.addEventListener("keydown", function (e) {
    var x = document.getElementById("suggestions");
    //select the li elementsof suggestions ul
